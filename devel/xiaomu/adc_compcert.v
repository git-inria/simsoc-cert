(**
SimSoC-Cert, a toolkit for generating certified processor simulators
See the COPYRIGHTS and LICENSE files.

Coq representation of a C program automatically generated by Simgen.
*)

Require Import ZArith Cnotations.

Open Scope Z_scope.

(* identifiers *)

Open Scope positive_scope.
Definition hi := 1.
Definition lo := 2.
Definition struct___int64 := 3.
Definition mem := 4.
Definition _end := 5.
Definition size := 6.
Definition begin := 7.
Definition SLv6_MMU := 8.
Definition background := 9.
Definition mode := 10.
Definition T_flag := 11.
Definition F_flag := 12.
Definition I_flag := 13.
Definition A_flag := 14.
Definition E_flag := 15.
Definition GE3 := 16.
Definition GE2 := 17.
Definition GE1 := 18.
Definition GE0 := 19.
Definition J_flag := 20.
Definition Q_flag := 21.
Definition V_flag := 22.
Definition C_flag := 23.
Definition Z_flag := 24.
Definition N_flag := 25.
Definition SLv6_StatusRegister := 26.
Definition v_bit := 27.
Definition u_bit := 28.
Definition ee_bit := 29.
Definition SLv6_SystemCoproc := 30.
Definition jump := 31.
Definition pc := 32.
Definition und_regs := 33.
Definition abt_regs := 34.
Definition svc_regs := 35.
Definition irq_regs := 36.
Definition fiq_regs := 37.
Definition user_regs := 38.
Definition id := 39.
Definition cp15 := 40.
Definition spsrs := 41.
Definition cpsr := 42.
Definition mmu_ptr := 43.
Definition SLv6_Processor := 44.
Definition gvars := 45.
Definition main := 46.
Definition shifter_operand := 47.
Definition n := 48.
Definition d := 49.
Definition cond := 50.
Definition S := 51.
Definition old_Rn := 52.
Definition proc := 53.
Definition OverflowFrom_add3 := 54.
Definition CarryFrom_add3 := 55.
Definition reg := 56.
Definition get_bit := 57.
Definition unpredictable := 58.
Definition spsr := 59.
Definition copy_StatusRegister := 60.
Definition CurrentModeHasSPSR := 61.
Definition set_reg_or_pc := 62.
Definition ConditionPassed := 63.
Definition ADC := 64.
Close Scope positive_scope.

(* structs and unions *)

Definition typ_SLv6_SystemCoproc := Tstruct SLv6_SystemCoproc 
F[
  ee_bit -: uint8; 
  u_bit -: uint8; 
  v_bit -: uint8].

Definition typ_SLv6_StatusRegister := Tstruct SLv6_StatusRegister 
F[
  N_flag -: uint8; 
  Z_flag -: uint8; 
  C_flag -: uint8; 
  V_flag -: uint8; 
  Q_flag -: uint8; 
  J_flag -: uint8; 
  GE0 -: uint8; 
  GE1 -: uint8; 
  GE2 -: uint8; 
  GE3 -: uint8; 
  E_flag -: uint8; 
  A_flag -: uint8; 
  I_flag -: uint8; 
  F_flag -: uint8; 
  T_flag -: uint8; 
  mode -: uint32; 
  background -: uint32].

Definition typ_SLv6_MMU := Tstruct SLv6_MMU 
F[
  begin -: uint32; 
  size -: uint32; 
  _end -: uint32; 
  mem -: `*` uint8].

Definition typ_SLv6_Processor := Tstruct SLv6_Processor 
F[
  mmu_ptr -: `*` typ_SLv6_MMU; 
  cpsr -: typ_SLv6_StatusRegister; 
  spsrs -: Tarray typ_SLv6_StatusRegister 5; 
  cp15 -: typ_SLv6_SystemCoproc; 
  id -: int8; 
  user_regs -: Tarray uint32 16; 
  fiq_regs -: Tarray uint32 7; 
  irq_regs -: Tarray uint32 2; 
  svc_regs -: Tarray uint32 2; 
  abt_regs -: Tarray uint32 2; 
  und_regs -: Tarray uint32 2; 
  pc -: `*` uint32; 
  jump -: uint8].

(* expression types *)

Definition T1 := uint32.
Definition T2 := Tfunction T[`*` typ_SLv6_Processor; uint8] uint32.
Definition T3 := typ_SLv6_Processor.
Definition T4 := uint8.
Definition T5 := Tfunction T[`*` typ_SLv6_Processor; uint32] uint8.
Definition T6 := typ_SLv6_StatusRegister.
Definition T7 := int32.
Definition T8 := Tfunction T[`*` typ_SLv6_Processor; uint8; uint32] void.
Definition T9 := void.
Definition T10 := Tfunction T[`*` typ_SLv6_Processor] uint8.
Definition T11 := Tfunction T[`*` typ_SLv6_StatusRegister; uint32] void.
Definition T12 := Tfunction T[uint32; uint32] uint8.

(* global variables *)

Definition gv_gvars :=
  {| gvar_info := Tarray uint8 0;
     gvar_init := [Init_int8 0];
     gvar_readonly := false;
     gvar_volatile := false |}.

Definition global_variables := [(gvars,gv_gvars)].

(* functions *)

Definition fun_ADC :=
  (ADC, Internal
{| fn_return := void;
     fn_params := [
proc -: `*` typ_SLv6_Processor; 
S -: uint8; 
cond -: int32; 
d -: uint8; 
n -: uint8; 
shifter_operand -: uint32];
     fn_vars := [
old_Rn -: uint32];
     fn_body :=
($ old_Rn`:T1) `= (call (\reg`:T2) E[\proc`:T3; \n`:T4] T1)`:T1;;
`if (call (\ConditionPassed`:T5) E[&((`*(\proc`:T3)`:T6)|cpsr`:T6)`:T6; \cond`:T7] T5)
then (call (\set_reg_or_pc`:T8) E[\proc`:T3; \d`:T4; ((\old_Rn`:T1)+(\shifter_operand`:T1)`:T1)+(valof (((`*(\proc`:T3)`:(`*` T3))|cpsr`:T6)|C_flag`:T4) T4)`:T1] T9);;
`if (((\S`:T4)==(#1`:T7)`:T7)?(((\d`:T4)==(#15`:T7)`:T7)?(#1`:T7)`:(#0`:T7)`:T7)`:(#0`:T7)`:T7)
then `if (call (\CurrentModeHasSPSR`:T10) E[\proc`:T3] T10)
then (call (\copy_StatusRegister`:T11) E[&((`*(\proc`:T3)`:T6)|cpsr`:T6)`:T6; (call (\spsr`:T6) E[\proc`:T3] T6)] T9)
else (call (\unpredictable`:T9) E[] T9)
else `if ((\S`:T4)==(#1`:T7)`:T7)
then (((`*(\proc`:T3)`:(`*` T3))|cpsr`:T6)|N_flag`:T4) `= (call (\get_bit`:T12) E[(call (\reg`:T2) E[\proc`:T3; \d`:T4] T1); #31`:T7] T4)`:T4;;
(((`*(\proc`:T3)`:(`*` T3))|cpsr`:T6)|Z_flag`:T4) `= (((call (\reg`:T2) E[\proc`:T3; \d`:T4] T1)==(#0`:T7)`:T7)?(#1`:T7)`:(#0`:T7)`:T7)`:T4;;
(((`*(\proc`:T3)`:(`*` T3))|cpsr`:T6)|C_flag`:T4) `= (call (\CarryFrom_add3`:T9) E[\old_Rn`:T1; \shifter_operand`:T1; (valof (((`*(\proc`:T3)`:(`*` T3))|cpsr`:T6)|C_flag`:T4) T4)] T4)`:T4;;
(((`*(\proc`:T3)`:(`*` T3))|cpsr`:T6)|V_flag`:T4) `= (call (\OverflowFrom_add3`:T9) E[(valof (\old_Rn`:T1) T1); \shifter_operand`:T1; (valof (((`*(\proc`:T3)`:(`*` T3))|cpsr`:T6)|C_flag`:T4) T4)] T4)`:T4
else skip
else skip |}).

Definition functions := [fun_ADC].

(* program *)

Definition p :=
  {| prog_funct := functions;
     prog_main := main;
     prog_vars := global_variables |}.


Definition fld_mmu :fieldlist:=
  F[
    begin -: uint32; 
    size -: uint32; 
    _end -: uint32; 
    mem -: `*` uint8].

Definition fld_sc :fieldlist:=
  F[
    ee_bit -: uint8; 
    u_bit -: uint8; 
    v_bit -: uint8].

Definition fld_sr :fieldlist:=
F[
  N_flag -: uint8; 
  Z_flag -: uint8; 
  C_flag -: uint8; 
  V_flag -: uint8; 
  Q_flag -: uint8; 
  J_flag -: uint8; 
  GE0 -: uint8; 
  GE1 -: uint8; 
  GE2 -: uint8; 
  GE3 -: uint8; 
  E_flag -: uint8; 
  A_flag -: uint8; 
  I_flag -: uint8; 
  F_flag -: uint8; 
  T_flag -: uint8; 
  mode -: uint32; 
  background -: uint32].

Definition fld_proc :fieldlist:=
F[
  mmu_ptr -: `*` typ_SLv6_MMU; 
  cpsr -: typ_SLv6_StatusRegister; 
  spsrs -: Tarray typ_SLv6_StatusRegister 5; 
  cp15 -: typ_SLv6_SystemCoproc; 
  id -: int8; 
  user_regs -: Tarray uint32 16; 
  fiq_regs -: Tarray uint32 7; 
  irq_regs -: Tarray uint32 2; 
  svc_regs -: Tarray uint32 2; 
  abt_regs -: Tarray uint32 2; 
  und_regs -: Tarray uint32 2; 
  pc -: `*` uint32; 
  jump -: uint8].

Definition ADC_body :function:=
{| fn_return := void;
     fn_params := [
proc -: `*` typ_SLv6_Processor; 
S -: uint8; 
cond -: int32; 
d -: uint8; 
n -: uint8; 
shifter_operand -: uint32];
     fn_vars := [
old_Rn -: uint32];
     fn_body :=
($ old_Rn`:T1) `= (call (\reg`:T2) E[\proc`:T3; \n`:T4] T1)`:T1;;
`if (call (\ConditionPassed`:T5) E[&((`*(\proc`:T3)`:T6)|cpsr`:T6)`:T6; \cond`:T7] T5)
then (call (\set_reg_or_pc`:T8) E[\proc`:T3; \d`:T4; ((\old_Rn`:T1)+(\shifter_operand`:T1)`:T1)+(valof (((`*(\proc`:T3)`:(`*` T3))|cpsr`:T6)|C_flag`:T4) T4)`:T1] T9);;
`if (((\S`:T4)==(#1`:T7)`:T7)?(((\d`:T4)==(#15`:T7)`:T7)?(#1`:T7)`:(#0`:T7)`:T7)`:(#0`:T7)`:T7)
then `if (call (\CurrentModeHasSPSR`:T10) E[\proc`:T3] T10)
then (call (\copy_StatusRegister`:T11) E[&((`*(\proc`:T3)`:T6)|cpsr`:T6)`:T6; (call (\spsr`:T6) E[\proc`:T3] T6)] T9)
else (call (\unpredictable`:T9) E[] T9)
else `if ((\S`:T4)==(#1`:T7)`:T7)
then (((`*(\proc`:T3)`:(`*` T3))|cpsr`:T6)|N_flag`:T4) `= (call (\get_bit`:T12) E[(call (\reg`:T2) E[\proc`:T3; \d`:T4] T1); #31`:T7] T4)`:T4;;
(((`*(\proc`:T3)`:(`*` T3))|cpsr`:T6)|Z_flag`:T4) `= (((call (\reg`:T2) E[\proc`:T3; \d`:T4] T1)==(#0`:T7)`:T7)?(#1`:T7)`:(#0`:T7)`:T7)`:T4;;
(((`*(\proc`:T3)`:(`*` T3))|cpsr`:T6)|C_flag`:T4) `= (call (\CarryFrom_add3`:T9) E[\old_Rn`:T1; \shifter_operand`:T1; (valof (((`*(\proc`:T3)`:(`*` T3))|cpsr`:T6)|C_flag`:T4) T4)] T4)`:T4;;
(((`*(\proc`:T3)`:(`*` T3))|cpsr`:T6)|V_flag`:T4) `= (call (\OverflowFrom_add3`:T9) E[(valof (\old_Rn`:T1) T1); \shifter_operand`:T1; (valof (((`*(\proc`:T3)`:(`*` T3))|cpsr`:T6)|C_flag`:T4) T4)] T4)`:T4
else skip
else skip |}.
