...
let lib b ps =
  ...
  in
    ...
    (* print preamble *)
    bprintf b
"(* File generated by SimSoC-Cert. It provides the definitions of the types\nof %s instructions, and their semantics. *)\n\nRequire Import Bitvec List Util Functions Config %s State Semantics ZArith Message.\n\n" (if ps.header = [] then "ARMv6 addressing modes and" else "SH4") (if ps.header = [] then "Arm SCC" else "");
