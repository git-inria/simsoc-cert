...
Definition semfun := list (string * word) -> bool -> state -> result.

Definition todo (m : message) (loc : list (string * word)) (b : bool) 
  (s : state) : result := Todo m.

Definition unpredictable (m : message) (loc : list (string * word)) 
  (b : bool) (s : state) : result := Ko m.

Definition if_then (c : bool) (f : semfun) (loc : list (string * word))
  (b : bool) (s : state) : result :=
  if c then f loc b s else Ok loc b s.

Definition if_then_else (c : bool) (f1 f2 : semfun) 
  (loc : list (string * word)) (b : bool) (s : state)
  : result := if c then f1 loc b s else f2 loc b s.

Definition seq (f1 f2 : semfun) (loc0 : list (string * word)) 
  (b0 : bool) (s0 : state) : result :=
...
Fixpoint block (fs : list semfun) (loc0 : list (string * word)) (b0 : bool) 
  (s0 : state) : result :=
  match fs with
    | nil => Ok loc0 b0 s0
    | f :: fs' =>
...
Definition loop (p q : nat) (f : nat -> semfun) (loc0 : list (string * word))
  (b0 : bool) (s0 : state)
  : result := loop_aux p (q - p + 1) f loc0 b0 s0.

Fixpoint update_loc_aux (s : string) (v : word) (loc : list (string * word))
  : list (string * word) :=
  match loc with
    | nil => ((s, v) :: loc)
...
Fixpoint get_loc (str : string) (loc : list (string * word)) : word :=
  match loc with
    | nil => zero
    | (s, v) :: locs => if string_dec s str then v else get_loc str locs
  end.

Fixpoint get_loc' (str : string) (loc : list (string * word)) : option word :=
  match loc with
    | nil => None
    | (s, v) :: locs => if string_dec s str then Some v else get_loc' str locs
  end.

Definition set_cpsr (v : word) (loc : list (string * word)) (b : bool) 
  (s: state) : result :=
  Ok loc b (set_cpsr s v).

Definition set_cpsr_bit (n : nat) (v : word) (loc : list (string * word))
  (b : bool) (s: state) : result :=
...
Definition set_reg (n : regnum) (v : word) 
  (loc : list (string * word)) (b : bool) (s : state) : result
...
Definition ClearExclusiveLocal (pid : word) (loc : list (string * word))
  (b : bool) (s : state) : result :=
